...
function initColorPrototype() {
    const x = new Color("X");
    const o =  new Color("O");
    const none = new Color("_");
    ...
    Color.get = function (ordinal) {
        ...
        assert(ordinal % 1 === 0)
        assert(new ClosedInterval(Color.values().length - 1).includes(ordinal));
        return Color.values()[ordinal];
    };

    Color.none = none;

    Color.prototype.isNone = function() {
        return this === none;
    };
    ...
    Color.prototype.ordinal = function() {
        assert(Color.values().includes(this));        
...
function initBoardPrototype() {
    ...
    Board.prototype.isWinner = function (color) {
        ...
        const directions = [vertical, horizontal, diagonal, inverseDiagonal];
